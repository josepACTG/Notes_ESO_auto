...
numeric_df_comprovar <- function(dades_alumnes){
  ...
  # es_numerico <- function(x_var) {
  #   # Primero verificamos si es un número con expresión regular
  #   if (grepl("^-?\\d+(\\.\\d+)?$", x_var)) {
  #     # Si pasa la regex, intentamos convertir a numérico
  #     return(!is.na(as.numeric(x_var)))
  #   } else {
  #     # Si no pasa la regex, directamente devolvemos FALSE
  #     return(FALSE)
  #   }
  # }
  ...
  if (sum(valores_numericos) ==   length(llista_valors) ){
    # valors_llista <- llista_valors[valores_numericos]
    # 
    # 
    # filas <- which(apply(dades_alumnes, 1, function(fila) any(fila %in% valors_llista)))
    # 
    # # print(names(filas))
    # 
    # shinyalert(
    #   title = "¡Error!",
    #   text = paste("Hi ha valors numèrics en les files:", paste(names(filas), collapse = ", ")  ),
    #   type = "error"
    # )
    return(TRUE)
  }
...
barrejat_df_comprovar <- function(dades_alumnes){
  ...
  es_numerico <- function(x) {
    num_valor <- suppressWarnings(as.numeric(convert_to_numeric(x)))
    !is.na(suppressWarnings(as.numeric(convert_to_numeric(x)))) & grepl("^-?\\d+(\\.\\d+)?$", x)
  }

  # Verificar si hay valores no numéricos en columnas de tipo carácter
  hay_no_numericos <- any(sapply(dades_alumnes, function(col) {
    if (is.character(col)) {
      
      # Suprimir avisos al aplicar la función es_numerico
      suppressWarnings(any(!es_numerico(convert_to_numeric(col))))
    } else {
      FALSE
    }
  }))
  ...
  valores_numericos <- sapply(unlist(na.omit(dades_alumnes)), es_numerico)
  
  if (length(valores_numericos) == 0){ valores_numericos <- 0}

  
  # Si todos los valores són numericos, o tienen potencial de serlo:
  if ( sum(valores_numericos) ==  length(na.omit(unlist(dades_alumnes)))){
    return("tot_numeric")
  } #Sino, si no todos los valores son numericos, pero hay numericos:
  else if (sum(valores_numericos) > 0){
    return("tot_barrejat")
  }
  #SI no és así, si no hay valores numéricos, será que todos són carácteres.
  else {
...
ui <- fluidPage(  # Define el diseño principal de la aplicación usando un diseño fluido (responsive).
  ...
  sidebarLayout(  # Crea un diseño con un panel lateral y un panel principal.
    ...
    sidebarPanel(  # Panel lateral donde se encuentran los controles de entrada y botones.
      ...
      checkboxInput("mostrar_comentaris", "En l'arxiu hi ha comentaris (última columna)", value = FALSE),              
...
server <- function(input, output, session) {
  ...
  observeEvent(input$fitxer, {
     ...
     tryCatch({
      ...
      # dades_alumnes_excel <- read_excel("D:\\Escritorio\\Notes_Auto_ESO\\Notes_alumnes_dificils.xlsx") # <- Ordinador casa
      ...
      #ÇÇÇ Comentaris: Guardar.
      # Si hi ha comentaris (última columna), els guarda.
      if (input$mostrar_comentaris){
        ...
        dades_reactives$comentaris <- dades_alumnes[,ncol(dades_alumnes), drop = FALSE]
        #drop = FALSE és per a no perdre els noms de les files.
      ...
      # Si totes les dades són numèriques:
      if (barrejat_df_comprovar(dades_alumnes) == "tot_barrejat"){
        print("Dades barrejades")
      ...
      else if (barrejat_df_comprovar(dades_alumnes) == "tot_numeric"){
      ...
      else if ( barrejat_df_comprovar(dades_alumnes) == "tot_caracters" ){
  ...
  output$grafic_densitats_1 <- renderPlotly({
    ...
    tryCatch({
      ...
      p <- suppressWarnings(
        ...
        geom_jitter(aes(color = colores, text = paste("Alumne: ", alumne, "<br>Nota: ", nota)), width = 0.2, size = 2) +
